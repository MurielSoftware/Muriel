...
namespace Muriel
{
	...
	private:
		int _lastChar;
		BYTE _keyStates[2][256];
		BYTE *_pCurrKeyStates;
		BYTE *_pPrevKeyStates;
	public:
		virtual ~Keyboard();
		...
		inline int GetLastChar() const { return _lastChar; }
		inline bool KeyDown(Key key) const { return (_pCurrKeyStates[key] & 0x80) ? true : false; }
